...
class MusicPlayer():
    def __init__(self):
        myLogger.debug('Initializing Music Player')
    ...
    async def init_client(self, first_channel):
        try:
            myLogger.debug('Initializing Client Connection')
            self.client = await first_channel.connect()
            myLogger.info('Voice Channel Connection Set')
        except:
            myLogger.critical('Voice Channel Connection Failed!')
    ...
    def play_next(self):
        ...
        if len(self.queue) > 0:
            myLogger.debug('found song(s) in queue. continuing.')
            self.isPlaying = True
            ...
            myLogger.info('NOW PLAYING: %s', self.currSong.title)

        else:
            myLogger.info('found empty queue. resetting and stopping.')
...
class MCog(commands.Cog):
    ...
    def active_permission_check(self, ctx):
        myLogger.debug('checking valid permissions')
        try:
            ...
            myLogger.debug('active permission check failed')
            return False
    ...
    async def first_connect(self, ctx):
        if ctx.guild.id in self.MUSIC_DATABASE: #guild already added
            ...
            myLogger.info('first connection in server. initializing services.')
            ...
            myLogger.info('Guild Recorded in Database, MusicPlayer Created, Client Initiated and Connected to Channel')
            return True
    ...
    def search_yt(self, args: tuple, num_results = 3):
        myLogger.info('searching youtube, querying %i results', num_results)
        error_msg = ''
        nb.mute_stdout()
...
#command function for play, queue, and playnow
    async def play_command(self, ctx, args: tuple, invocation, num_results: int = 3):
        myLogger.debug('called play_command()')
        ...
        try:
            if not await self.first_connect(ctx) and ctx.author.voice.channel != self.MUSIC_DATABASE[ctx.guild.id].client.channel:
                ...
                myLogger.info('client successfully moved to new channel')
        except Exception as err:
            myLogger.info("failed. possibilities: bad first connect || couldn't switch channels")
    ...
    async def task_updateControlPanel(self):
        myLogger.debug('beginning tasks.loop to update control panel')
    ...
    async def task_databaseCleanup(self):
        ...
        for guildId, player in self.MUSIC_DATABASE.items():
            if not (player.isPlaying or player.client.is_connected()):
                myLogger.info('deleting music player in guild %s', guildId)
                del self.MUSIC_DATABASE[guildId]

        if len(self.MUSIC_DATABASE) > 0:
            myLogger.debug('restarting tasks.loop')
